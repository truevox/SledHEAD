...
let cameraX = 0;
let cameraTargetX = 0;
const cameraLerpFactor = 0.08; // Slightly reduced for smoother transitions
...
function drawBackground() {
  // Get the current game state to determine color scheme
  const isDownhill = window.currentState === window.GameState.DOWNHILL;
  
  // Background gradient colors
  const topColor = isDownhill ? "#87CEEB" : "#ADD8E6"; // Sky blue
  const bottomColor = isDownhill ? "#ADD8E6" : "#98FB98"; // Light blue/green
...
/**
 * Draws a subtle fog/fade effect at the edges of the screen to mask wrapping
 */
function drawEdgeFogEffect() {
  ...
  rightGradient.addColorStop(1, 'rgba(255, 255, 255, 0.3)');
